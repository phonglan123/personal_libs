function KeywordFromStringOrHtml(string_or_html, maincontent_queryselector, default_document = document) {
    const
        ...
        dom_parser = (html_string, doc) => {
            let elm = doc.createElement("html");
            elm.innerHTML = html_string;
            return elm;
        },
        get_main_content = (dom, queryselector, doc) => {
            if (queryselector != undefined)
                return dom.querySelector(queryselector).innerText.toLowerCase();

            function readability_runthis(dom = document) {
                var dbg = function (s) {
                    if (typeof console !== 'undefined')
                        return false; //console.log("Readability: " + s);
                };
                ...
                **/
                var readability = {
                    ...
                    init: function (preserveUnlikelyCandidates) {
                        ...
                        if (dom.body && !readability.bodyCache)
                            readability.bodyCache = dom.body.innerHTML;

                        readability.prepDocument();

                        /* Build readability's DOM tree */
                        var overlay = dom.createElement("DIV");
                        var innerDiv = dom.createElement("DIV");
                        ...
                        **/
                        if (readability.getInnerText(articleContent, false) == "") {
                            if (!preserveUnlikelyCandidates) {
                                dom.body.innerHTML = readability.bodyCache;
                        ...
                        dom.body.className = readability.readStyle;
                        ...
                        dom.body.innerHTML = "";
                        dom.body.insertBefore(overlay, dom.body.firstChild);

                        if (readability.frameHack) {
                            var readOverlay = dom.getElementById('readOverlay');
                    ...
                    getArticleTools: function () {
                        var articleTools = dom.createElement("DIV");
                    ...
                    /**
                     * Get the article title as an H1. Currently just uses dom.title,
                     * we might want to be smarter in the future.
                     *
                     * @return void
                     **/
                    getArticleTitle: function () {
                        var articleTitle = dom.createElement("H1");
                        articleTitle.innerHTML = dom.title;
                    ...
                    getArticleFooter: function () {
                        var articleFooter = dom.createElement("DIV");
                    ...
                    prepDocument: function () {
                         ...
                         * so we create a new body node and append it to the dom.
                         */
                        if (dom.body === null) {
                        ...
                        if (frames.length > 0) {
                            ...
                            for (var frameIndex = 0; frameIndex < frames.length; frameIndex++) {
                                ...
                                try {
                                    frames[frameIndex].contentWindow.dom.body;
                                    canAccessFrame = true;
                                } catch (e) { }
                            ...
                            if (bestFrame) {
                                var newBody = dom.createElement('body');
                                newBody.innerHTML = bestFrame.contentWindow.dom.body.innerHTML;
                                newBody.style.overflow = 'scroll';
                                dom.body = newBody;

                                var frameset = dom.getElementsByTagName('frameset')[0];
                                if (frameset)
                        ...
                        if (readability.readStyle == "style-classy") {
                            var typeKitScript = dom.createElement('script');
                            typeKitScript.type = "text/javascript";
                            typeKitScript.src = "http://use.typekit.com/sxt6vzy.js";

                            dom.body.appendChild(typeKitScript);

                            /**
                             * Done as a script elem so that it's ensured it will activate
                             * after typekit is loaded from the previous script src.
                            **/
                            var typeKitLoader = dom.createElement('script');
                            typeKitLoader.type = "text/javascript";

                            var typeKitLoaderContent = dom.createTextNode('try{Typekit.load();}catch(e){}');
                            typeKitLoader.appendChild(typeKitLoaderContent);
                            dom.body.appendChild(typeKitLoader);
                        }
                        ...
                        for (var k = 0; k < dom.styleSheets.length; k++) {
                            if (dom.styleSheets[k].href != null && dom.styleSheets[k].href.lastIndexOf("readability") == -1) {
                                dom.styleSheets[k].disabled = true;
                            }
                        }

                        /* Remove all style tags in head (not doing this on IE) - TODO: Why not? */
                        var styleTags = dom.getElementsByTagName("style");
                        ...
                        dom.body.innerHTML = dom.body.innerHTML.replace(readability.regexps.replaceBrsRe, '</p><p>').replace(readability.regexps.replaceFontsRe, '<$1span>')
                    },
                    ...
                    **/
                    grabArticle: function (preserveUnlikelyCandidates) {
                        ...
                        **/
                        for (var nodeIndex = 0; (node = dom.getElementsByTagName('*')[nodeIndex]); nodeIndex++) {
                            ...
                            if (node.tagName === "DIV") {
                                if (node.innerHTML.search(readability.regexps.divToPElementsRe) === -1) {
                                    ...
                                    var newNode = dom.createElement('p');
                                    try {
                        ...
                        **/
                        var allParagraphs = dom.getElementsByTagName("p");
                        var candidates = [];
                        ...
                        **/
                        ...
                        if (topCandidate == null || topCandidate.tagName == "BODY") {
                            ...
                            topCandidate.innerHTML = dom.body.innerHTML;
                            dom.body.innerHTML = "";
                            dom.body.appendChild(topCandidate);
                        ...
                        **/
                        var articleContent = dom.createElement("DIV");
                    ...
                    **/
                     ...
                     **/
                    emailBox: function () {
                        var emailContainer = dom.getElementById('email-container');
                        if (null != emailContainer) {
                            return;
                        }

                        var emailContainer = dom.createElement('div');
                        emailContainer.setAttribute('id', 'email-container');
                        emailContainer.innerHTML = '<iframe src="' + readability.emailSrc + '?pageUrl=' + escape(window.location) + '&pageTitle=' + escape(dom.title) + '" scrolling="no" onload="readability.removeFrame()" style="width:500px; height: 490px; border: 0;"></iframe>';

                        dom.body.appendChild(emailContainer);
                    },
                     ...
                     **/
                    kindleBox: function () {
                        var kindleContainer = dom.getElementById('kindle-container');
                        if (null != kindleContainer) {
                            return;
                        }

                        var kindleContainer = dom.createElement('div');
                        ...
                        dom.body.appendChild(kindleContainer);
                        ...
                        var formHtml = '<form id="readabilityKindleForm" style="display: none;" target="readabilityKindleIframe" method="post" action="' + readability.kindleSrc + '">\
                                                 <input type="hidden" name="bodyContent" id="bodyContent" value="' + readability.htmlspecialchars(dom.getElementById('readability-content').innerHTML) + '" />\
                                                 <input type="hidden" name="pageUrl" id="pageUrl" value="' + readability.htmlspecialchars(window.location) + '" />\
                                                 <input type="hidden" name="pageTitle" id="pageUrl" value="' + readability.htmlspecialchars(dom.title) + '" />\
                                             </form>';

                        dom.body.innerHTML += formHtml;
                        dom.forms['readabilityKindleForm'].submit();
                    },
            ...
            let temp_frame = doc.createElement("iframe"),
                temp_frame_doc = undefined,
                temp_frame_window = undefined,
            ...
            temp_frame_window = temp_frame.contentWindow;

            temp_frame_doc.head.innerHTML = dom.querySelector("head").innerHTML;
            temp_frame_doc.body.innerHTML = dom.querySelector("body").innerHTML;

            temp_frame_script = temp_frame_doc.createElement("script");
            temp_frame_script.innerHTML = readability_runthis.toString() + "readability_runthis();";
            temp_frame_doc.body.appendChild(temp_frame_script);
            temp_frame_window.readability_runthis();

            temp_frame_readfooter = temp_frame_doc.querySelector("#readability-content #readFooter");
            temp_frame_readfooter.parentNode.removeChild(temp_frame_readfooter);
            return_result = temp_frame_doc.querySelector("#readability-content").innerText.toLowerCase();
